...
# PYTHONPATH=/tmp/Megatron-LM python src/transformers/models/rnabert/convert_rnabert_checkpoint.py ...
#
...
import torch
from . import RNABertModel, RNABertConfig, RNABertForMaskedLM
import warnings
...
def load_mp_state_dict(base_dir):
    ...
    for mp in mp_list:
        ...
        if len(master_state_dict) == 0:
            ...
            master_state_dict["model"]["language_model"]["encoder"].update(new_layers)
...
def main():
    ...
    if args.path_to_checkpoint.endswith(".zip"):
        ...
        if args.path_to_checkpoint.endswith(".pt"):
            print("> Found .pt, converting model, no pipeline parallelism")
            ## no pipeline parallelism
            input_state_dict = torch.load(
                args.path_to_checkpoint, map_location="cuda:0"
            )
        else:
            print(
                "> Found folder, converting with pipeline parallelism assumption (not tensor parallelism)"
            )
            # works with pipeline parallelism, no tensor parallelism
            input_state_dict = load_mp_state_dict(args.path_to_checkpoint)

        ## the old one not for pipeline parallelism
        # input_state_dict = torch.load(args.path_to_checkpoint, map_location="cuda:0")

    is_mcore_model = input_state_dict["args"].use_mcore_models

    if args.config_file == "":
        # Default config of rnabert 345m
        config = RNABertConfig()

        # different rnabert-*-345m models have different vocab sizes, so override the default
        # config (which is for rnabert-cased-345m) with the actual vocab dimension
        if is_mcore_model:
            config.vocab_size = input_state_dict["model"]["output_layer.bias"].size(0)
        else:
            config.vocab_size = input_state_dict["model"]["lm_head"]["bias"].numel()
    else:
        config = RNABertConfig.from_json_file(args.config_file)
    ...
    config = RNABertConfig(
        ...
        add_linear_bias=megatron_args.add_bias_linear,
        position_embedding_type=(
            "rope" if megatron_args.use_rotary_position_embeddings else "absolute"
        ),
    )
